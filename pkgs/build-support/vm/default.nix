...
rec {
  ...
  stage1Init = writeScript "vm-run-stage1" ''
    ...
    mkdir -p /fs${buildDir}
    mount -t 9p sa /fs${buildDir} -o trans=virtio,version=9p2000.L,cache=loose,msize=131072
  ...
  stage2Init = writeScript "vm-run-stage2" ''
    #! ${bash}/bin/sh
    set -euo pipefail
    source ${buildDir}/xchg/saved-env
  ...
  qemuCommandLinux = ''
    ${if (customQemu != null) then customQemu else (qemu-common.qemuBinary qemu)} \
      ...
      -virtfs local,path=${buildDir},security_model=none,mount_tag=sa \
